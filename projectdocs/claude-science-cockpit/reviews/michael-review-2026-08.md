...
| 4 | `say.go` infers reproduction level (`L1` whenever `via != ""`) instead of parsing plankton's actual `reproduction: <level>` line — mislabels genuine L0 reproductions as L1 and then rejects them against an `L0` policy | Not started. |
